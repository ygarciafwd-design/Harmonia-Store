...
/** Campos del formulario de producto (estado del modal). precio/stock como string en inputs o number. */
type ProductoCampos = {
  nombre: string;
...
/** Errores de validación por campo (mensaje en string). */
type ProductoErrores = Partial<Record<keyof ProductoCampos, string>>;

/** Producto final enviado al guardar (precio y stock como number). */
type ProductoGuardado = Omit<ProductoCampos, 'precio' | 'stock'> & { precio: number; stock: number };

type Props = {
  producto: ProductoCampos;
  onGuardar: (producto: ProductoGuardado) => void;
  onCerrar: () => void;
  esNuevo?: boolean;
};

const ModalEditarProducto = ({ producto, onGuardar, onCerrar, esNuevo = false }: Props) => {
  const [campos, setCampos] = useState<ProductoCampos>({ ...producto });
  const [errores, setErrores] = useState<ProductoErrores>({});
  const [modoImagen, setModoImagen] = useState<'url' | 'archivo'>(
  ...
  const handleGuardar = async () => {
    ...
    const productoFinal: ProductoGuardado = {
      ...campos,
  ...
  return (
    <div className="modal-overlay" onClick={onCerrar}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <CampoFormulario label="Precio ($) *" tipo="number" nombre="precio" valor={String(campos.precio ?? '')} onChange={handleChange} error={errores.precio} placeholder="Ej: 250" />
        ...
        <CampoFormulario label="Stock (unidades) *" tipo="number" nombre="stock" valor={String(campos.stock ?? '')} onChange={handleChange} error={errores.stock} placeholder="Ej: 10" />
